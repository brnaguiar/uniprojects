...
static void createProcess(void* (*func)(void*), void* arg, pid_t *p);

pid_t* barber_process;
...
int main(int argc, char* argv[])
{
   ...
   *global = params;
   ...
   //iniciar semaforo controlo barber bench
   //sem_t semaforo inicia o semaforo no endereço apontado como argumento
   //pshare indica se este semaforo pode ser partilhado por threads ou processos
   //value indica o valor inicial do semeforo
   //psem_init(&shop, 1, 1);
...
static void go()
{
   ...
   //create process para cada barbeiro
   barber_process = (pid_t*)mem_alloc(sizeof(pid_t) * global->NUM_BARBERS);
   for(int i=0; i<global->NUM_BARBERS; i++){
     createProcess(main_barber, allBarbers+i, &barber_process[i]);
   }
   client_process = (pid_t*)mem_alloc(sizeof(pid_t) * global->NUM_CLIENTS);
   for(int i = 0; i < global->NUM_CLIENTS; i++) {
      createProcess(main_client, allClients+i,&client_process[i]);
   }
...
//cria process filho
//fork-chama uma vez e retorna 2;
//Um retorno é para o processo parent e devolve o PID do processo filho
//o segundo retorno é para o processo filho e devolve 0
static void createProcess(void* (*func)(void*), void* arg, pid_t *p){
    pid_t pid = pfork();
    if(pid==0){ //filho
      func(arg);
...
static void finish()
{
   ...
   pid_t pid;
   int status;
   while ((pid=pwaitpid(-1, &status, 0))!=-1);
...
static void initSimulation()
{
   ...
   //criar memoria partilhada - System V
   //shmid-share memory id
   //nao tenho certeza da key
   shmid_shop = pshmget(IPC_PRIVATE, sizeof(BarberShop), 0600 | IPC_CREAT | IPC_EXCL);
   //mapear bloco para espaço de endereçamento do processo e obter um apontador para esse bloco
   //retorna um apontador genérico para o inicio do bloco partilhado no caso de sucesso; -1 erro
   shop = (BarberShop*)pshmat(shmid_shop, NULL, 0);
   ...
   //nao tenho certeza da key
   shmid_barber = pshmget(IPC_PRIVATE, sizeof_barber()*global->NUM_BARBERS, 0600 | IPC_CREAT | IPC_EXCL);
   //mapear bloco para espaço de endereçamento do processo e obter um apontador para esse bloco
   //retorna um apontador genérico para o inicio do bloco partilhado no caso de sucesso; -1 erro
   allBarbers = (Barber*)pshmat(shmid_barber, NULL, 0);
   for(int i = 0; i < global->NUM_BARBERS; i++)
   ...
   //nao tenho certeza da key
   shmid_client = pshmget(IPC_PRIVATE, sizeof_client()*global->NUM_CLIENTS, 0600 | IPC_CREAT | IPC_EXCL);
   //mapear bloco para espaço de endereçamento do processo e obter um apontador para esse bloco
   //retorna um apontador genérico para o inicio do bloco partilhado no caso de sucesso; -1 erro
   allClients = (Client*)pshmat(shmid_client, NULL, 0);
   for(int i = 0; i < global->NUM_CLIENTS; i++)
