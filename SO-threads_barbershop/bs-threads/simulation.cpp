...
pthread_t* barb = (pthread_t*)mem_alloc(MAX_BARBERS*sizeof(pthread_t));
pthread_t* client = (pthread_t*)mem_alloc(MAX_CLIENTS*sizeof(pthread_t));
...
static void go()
{
   ...
   for(int i = 0; i < global->NUM_CLIENTS; i++){
      thread_create(&client[i], NULL,  main_client, (void *) &allClients[i]);
   }

   for(int i = 0; i < global->NUM_BARBERS; i++){
      thread_create(&barb[i], NULL, main_barber, (void *) &allBarbers[i]);
   }

   int k_barb;
   for(k_barb = 0; k_barb < MAX_BARBERS; k_barb++) {
     pthread_join(barb[k_barb], NULL);
   }

   int k_client;
   for (k_client = 0; k_client < MAX_CLIENTS; k_client++) {
     pthread_join(client[k_client], NULL);
  }
