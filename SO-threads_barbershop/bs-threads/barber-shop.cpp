...
static pthread_mutex_t clientbenches = PTHREAD_MUTEX_INITIALIZER;
...
static pthread_cond_t clientbenchesnotfull   = PTHREAD_COND_INITIALIZER;
static pthread_cond_t clientbenchesnotempty  = PTHREAD_COND_INITIALIZER;
                      ...
                      int num_client_benches_seats, int num_client_benches)
{
   ...
   pthread_mutex_lock(&clientbenches);
   ...
   pthread_cond_signal(&clientbenchesnotfull);
   pthread_mutex_unlock(&clientbenches);



}

void barber_shop(BarberShop* shop)
{
   require (shop != NULL, "shop argument required");
...
{
   /** TODO:
    * function called from a barber, when reserving a empty barber chair //mutex
    **/
   ...
   while (num_available_barber_chairs(shop) <= 0)
    {
        pthread_cond_wait(&barberchairsnotfull, &barberchairs);
    }
...
int reserve_random_empty_washbasin(BarberShop* shop, int barberID)
{
   ...
   while (num_available_washbasin(shop) <= 0)
    {
...
Service wait_service_from_barber(BarberShop* shop, int barberID)
{
   ...
   for(int i=1;i<MAX_CLIENTS;i++){
    if(shop->ClientServiceAssoc[i].barberID == barberID){
     res = shop->ClientServiceAssoc[i];
      shop->ClientServiceAssoc[i].washbasin = -1;
      shop->ClientServiceAssoc[i].clientID = 0;
      shop->ClientServiceAssoc[i].barberID = 0;
      shop->ClientServiceAssoc[i].pos = -1;
      shop->ClientServiceAssoc[i].request = 0;
      shop->ClientServiceAssoc[i].barberChair = -1;
      break;
    }
   }
   return res;
...
void inform_client_on_service(BarberShop* shop, Service service)
{
   ...
   require (shop != NULL, "shop argument required");
   for(int i = 1;i<=(MAX_CLIENTS);i++){
    if(i == service.clientID){
     shop->ClientServiceAssoc[i] = service;
      nextServiceFlag = 1;
      break;
    }
   }
...
void client_done(BarberShop* shop, int clientID)
{
   ...
   leave_barber_shop(shop,clientID);
...
int enter_barber_shop(BarberShop* shop, int clientID, int request)
{
   ...
   while (num_available_benches_seats(client_benches(shop)) <= 0)
    pthread_cond_wait(&clientbenchesnotfull, &clientbenches);

   int res = random_sit_in_client_benches(&shop->clientBenches, clientID, request);     //mutex locked

   pthread_cond_signal(&clientbenchesnotempty);
...
void leave_barber_shop(BarberShop* shop, int clientID)
{
   ...
   // o cliente afecta o numero de lugares livres? se sim meter mutexes e condicções de "benches not full"
...
void receive_and_greet_client(BarberShop* shop, int barberID, int clientID)
{
   ...
   require (shop != NULL, "shop argument required");
   require (barberID > 0, concat_3str("invalid barber id (", int2str(barberID), ")"));      //mudar
   require (clientID > 0, concat_3str("invalid client id (", int2str(clientID), ")"));


   for(int i=1;i<MAX_CLIENTS;i++){
    if(i == clientID){
      shop->ClientBarberAssoc[i] = barberID;
