...
static void notify_client_death(Client* client)
{
   ...
   client->state = DONE;
   global->NUM_CLIENTS -= 1;
   if(global->NUM_CLIENTS == 0){
    close_shop(client->shop);
   }
   log_client(client);
}
...
static void wandering_outside(Client* client)
{
    ...
    * 2. random a time interval [global->MIN_OUTSIDE_TIME_UNITS, global->MAX_OUTSIDE_TIME_UNITS]     done????
    **/
...
static int vacancy_in_barber_shop(Client* client)
{
   ...
   mutex_lock(&clientbenches); //------------------------------------------------
   
   if(num_available_benches_seats(client_benches(client->shop)) == 0){
      client->state = WAITING_BARBERSHOP_VACANCY;                       //mutex locked 
      res = 0;
   }
   mutex_unlock(&clientbenches); //------------------------------------------------
   return res;
}
...
static void wait_its_turn(Client* client)
{
  ...
  while(1){
    //log_client(client);
    //mutex_lock(&client->shop->clientbenches); //-----------------------------------------------------------------------------------------
    if((num_available_benches_seats(client_benches(client->shop))>0) and (!is_client_inside(client->shop, client->id))){       //nao tenho a certeza deste AQUI
    ...
    //mutex_unlock(&client->shop->clientbenches); //---------------------------------------------------------------------------------------
    }
  }

   while(client->shop->received != 1){}
    client->shop->received = 0;
...
static void rise_from_client_benches(Client* client)
{
    ...
    **/
   require (client != NULL, "client argument required");          //done???
   require (client != NULL, "client argument required");
    ...
    s = wait_service_from_barber(client->shop, client->barberID);
      ...
      log_client(client);
          if(s.request == HAIRCUT_REQ){
            ...
            sit_in_barber_chair(barber_chair(client->shop,client->chairPosition), s.clientID);      //mutex locked
            ...
            rise_from_barber_chair(barber_chair(client->shop,client->chairPosition), client->id);   //mutex locked
            
            mutex_unlock(&barberchairs); //-------------------------------------------------------------------

            s.request -= HAIRCUT_REQ;
        ...
        //  client->state = WAITING_SERVICE_START;
          ...
          else if(s.request == WASH_HAIR_REQ){
            client->chairPosition =  -1;
            client->benchesPosition = -1;
            ...
            sit_in_washbasin(washbasin(client->shop,client->basinPosition), s.clientID);          //mutex locked
            ...
            rise_from_washbasin(washbasin(client->shop,client->basinPosition), client->id);   //mutex locked
            
            mutex_unlock(&washbasins);  //---------------------------------------------------------------------------

            s.request -= WASH_HAIR_REQ;
