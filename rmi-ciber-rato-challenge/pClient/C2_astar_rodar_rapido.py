...
class MyRob(CRobLinkAngs):
    ...
    def __init__(self, rob_name, rob_id, angles, host):
        ...
        # self.mapping_neighborhood = []
        # self.mapping_newtowalk_cells = []
        # self.mapping_visited_cells = []
        # self.mapping_newtowalk_neighborhood = []
    ...
    def init_position(self):
        ...
        #self.mapping_visited_cells.append(self.curr_mapping) if self.curr_mapping not in self.mapping_visited_cells else _
        ...
        #self.mapping_newtowalk_cells.extend(mapping_neighbors)

        self.neighborhood = self.newtowalk_cells
        #self.mapping_neighborhood = self.mapping_newtowalk_cells

        self.newtowalk_neighborhood = self.newtowalk_cells
        #self.mapping_newtowalk_neighborhood = self.mapping_newtowalk_cells
    ...
    def update_position(self, diff, cardinal):
        ...
        #self.mapping_visited_cells.append(self.curr_mapping) if self.curr_mapping not in self.mapping_visited_cells else print("")
        ...
        #self.mapping_newtowalk_cells.extend([c for c in mapping_neighbors if c not in self.mapping_newtowalk_cells and c not in self.mapping_visited_cells])
        ...
        #self.mapping_newtowalk_cells.sort(key=lambda new_cell: new_cell[0] - self.curr_mapping[0] or new_cell[1] - self.curr_mapping[1]) ## sort pela distancia a celula de onde está.  newtowalk_cells

        self.neighborhood = neighbors  ## all neighbors of the cell.
        #self.mapping_neighborhood = mapping_neighbors
        ...
        #self.mapping_newtowalk_neighborhood = [fcell for fcell in self.mapping_neighborhood if fcell in self.mapping_newtowalk_cells] ## neighbors of the current cell yet to explore

        
        self.map_connections = self.map_connections + [[self.curr_cell, neighbor_cell] for neighbor_cell in self.neighborhood if [self.curr_cell, neighbor_cell] not in self.map_connections]          
    ...
    def choose_next(self, state):
        ...
        if self.tree_path == []:
            ...
            # print("")
            # print("OBJECTIVO: ", self.newtowalk_cells[0])
            # print("")
            # print("VISITADAS: ", self.visited_cells)
            # print("")
            # print("CONHECIDAS: ", self.newtowalk_cells)
            # print("")        


            for i in range(0, len(self.map_connections)-1):
                if self.map_connections[i] in self.map_connections[i+1:]:
                    print("CELULA ({}, {}) REPETIDA".format(self.map_connections[i][0], self.map_connections[i][1]))  

            print("LEN CONNECTIONS: ", len(self.map_connections))       

            self.tree_path = SearchTree(SearchProblem(Domain(self.map_connections), self.curr_cell, self.newtowalk_cells[0]), 'a*').search()[0]
            # if [self.newtowalk_cells[0]] not in self.tree_path:
            #     self.tree_path += [self.newtowalk_cells[0]]
            print("TREE: ", self.tree_path)        
            print(" " )    
    ...
    def move_one(self):
        ...
        if (cardinal == "N" or cardinal == "S"):
            ...
            if abs(round(self.measures.y,1) - self.next_cell[1]) == 0 or self.measures.irSensor[0]> 2:      #and self.measures.irSensor[0] < 2:
                #print("CHEGUEI")
                #print(" ")
